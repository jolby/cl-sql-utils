  ...
  (let* ((args (clingon:command-arguments cmd))
    ...
    (let ((tables (squ:table-names db :fts4 fts4 :fts5 fts5)))
      (dolist (table tables)
        ...
        (when columns
          (let ((cols (mapcar #'squ:name (squ:columns (squ:make-table db table)))))
...
(defun tables/command ()
  ...
  (clingon:make-command
   ...
   :examples '(("List tables:" . "sql-utils tables data.db"))))
...
(defun rows/command ()
  ...
  (clingon:make-command
   ...
   :examples '(("Show all rows:" . "sql-utils rows data.db mytable")
              ("Filter rows:" . "sql-utils rows data.db mytable --where \"id > 5\""))))

(defun top-level/options ()
  "Returns options for the top-level command"
  (list))
