...
(defgeneric make-db-connection (db-designator &rest args &key &allow-other-keys))
...
(defgeneric create-table-sql (database table-name columns &key pk foreign-keys column-order not-null defaults hash-id hash-id-columns
                                                            extracts if-not-exists replace ignore transform strict))
(defgeneric create-table (database table-name columns &key pk foreign-keys column-order not-null defaults hash-id hash-id-columns
                                                            extracts if-not-exists replace ignore transform strict))
...
(defgeneric row-count-where (queryable &rest args &key where where-args)
  (:documentation "Returns count of rows matching the where clause. If no where clause provided, counts all rows.")
  (:method ((q queryable) &rest args &key where where-args)
    (let* ((sql (format nil "SELECT COUNT(*) FROM [~A]~@[ WHERE ~A~]"
                        (name q)
                        where))
...
(defgeneric rows (queryable &rest args &key order-by select limit offset)
  (:documentation "Returns all rows from this queryable object as a list of property lists.")
  (:method ((q queryable) &rest args &key order-by select limit offset)
    (apply #'rows-where (append (list q) args))))

(defgeneric rows-where (queryable &key where where-args order-by select limit offset)
  (:documentation "Returns rows matching the where clause. Returns property lists for each row.")
  (:method ((q queryable) &rest args &key where where-args order-by select limit offset)
    (let* ((sql (with-output-to-string (s)
                  (format s "SELECT ~A FROM [~A]"
                          (or select "*")
                          (name q))
                  (when where
...
(defgeneric pks (queryable)
  (:documentation "Returns a list of column objects that make up the primary key"))
(defgeneric pk-names (queryable)
  (:documentation "Returns a list of column names that make up the primary key"))
...
(defgeneric create (table columns &key pk foreign-keys column-order not-null defaults hash-id hash-id-columns
                                    extracts if-not-exists replace ignore transform strict))
...
(defgeneric transform (table &key types rename drop pk not-null defaults drop-foreign-keys))
(defgeneric create-index (table columns &key index-name unique if-not-exists find-unique-name analyze))
(defgeneric add-column (table col-name &key col-type fk fk-col not-null-default))
...
(defgeneric insert-chunk (table alter extracts chunk all-columns hash-id hash-id-columns upsert pk))
(defgeneric insert (table record &key pk foreign-keys column-order not-null defaults hash-id hash-id-columns))
(defgeneric insert-all (table records &key pk foreign-keys column-order not-null defaults batch-size hash-id hash-id-columns upsert))
(defgeneric analyze-column (table column &key common-limit value-truncate total-rows most-common least-common))
