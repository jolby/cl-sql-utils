...
(defun convert-foreign-keys (table-name foreign-keys)
  ...
  (when foreign-keys
    (mapcar (lambda (fk)
              (destructuring-bind (column other-table &optional (other-column "id")) fk
                (make-foreign-key table-name column other-table other-column)))
            foreign-keys)))

(defmethod create-table-sql ((db database) table-name columns &key pk foreign-keys column-order not-null defaults
                                                                 hash-id hash-id-columns extracts if-not-exists
                                                                 replace ignore transform strict)
  "Generate SQL to create a table with the specified schema.
   Returns the SQL string needed to create the table."
  (declare (ignorable column-order hash-id hash-id-columns extracts ignore transform strict))
  (let* ((if-not-exists-sql (when if-not-exists " IF NOT EXISTS"))
         (replace-sql (when replace " OR REPLACE "))
         (column-defs '())
         (pk-constraint nil)
         (fk-constraints '()))
    ...
    (loop for (col-name . col-type) in columns
          for col-sql = (format nil "~A ~A~A~A"
                               (sql-quote db col-name)
                               (if (stringp col-type)
                                   col-type
                                   "TEXT")  ; Default to TEXT if no type specified
                               (if (and not-null (member col-name not-null :test #'string=))
                                   " NOT NULL"
                                   "")
                               (if (and defaults (assoc col-name defaults :test #'string=))
                                   (format nil " DEFAULT ~A"
                                           (quote-default-value db (cdr (assoc col-name defaults :test #'string=))))
                                   ""))
    ...
    (when pk
      (setf pk-constraint
            (if (listp pk)
                (format nil "PRIMARY KEY(~{~A~^, ~})" (mapcar (lambda (col) (sql-quote db col)) pk))
    ...
    (when foreign-keys
      (loop for fk in foreign-keys
            ...
            for constraint = (format nil "FOREIGN KEY(~a) REFERENCES ~a(~a)"
                                   (sql-quote db (car fk))
                                   (sql-quote db other-table)
                                   (sql-quote db (or other-col "id")))
            do (push constraint fk-constraints)))

    ;; Combine all parts into final SQL
    (format nil "CREATE~A TABLE~A ~A (~{~A~^, ~}~:[~;, ~]~A~:[~;, ~]~{~A~^, ~})"
            (or replace-sql "")
            (or if-not-exists-sql "")
            (sql-quote db table-name)
            (nreverse column-defs)
            pk-constraint
            pk-constraint
            (and fk-constraints t)
            (nreverse fk-constraints))))

(defmethod create-table ((db database) table-name columns &key pk foreign-keys column-order not-null defaults
                                                             hash-id hash-id-columns extracts if-not-exists
                                                             replace ignore transform strict)
  ...
  (let ((existing-table (make-table db table-name)))
    (cond
      ;; Table exists - handle according to flags
       ...
       (cond
         ...
         (replace 
          ...
          (create-table db table-name columns 
                       :pk pk :foreign-keys foreign-keys 
                       :column-order column-order :not-null not-null
                       :defaults defaults :hash-id hash-id
         ...
         (t
          ;; Create new table
          (let ((sql (create-table-sql db table-name columns
                                      :pk pk
