...
(defmethod initialize-instance :after
           ((db sqlite-database) &key filename memory-name memory recreate)
  (let ((connection-spec
         `(:sqlite3 :database-name
           ,(cond (memory-name ":memory:")
                  (memory ":memory:")
                  (filename filename)
                  (t
                   (error
                     "Either specify a filename or :memory t or :memory-name NAME"))))))
...
(defmethod make-db-connection
           ((db-designator (eql :sqlite))
            &rest args
            &key filename memory memory-name recreate)
  (declare (ignore args))
...
(defmethod make-table ((db sqlite-database) table-name &rest kwargs)
  ...
  (if (and (not (getf kwargs :force-new t))
      ...
      (let ((table
             (make-instance 'sqlite-table
                            :db db
                            :name table-name
                            :pk (getf kwargs :pk)
                            :foreign-keys (convert-foreign-keys table-name
                                                                (getf kwargs
                                                                      :foreign-keys))
...
(defmethod make-table-from-db ((db sqlite-database) table-name &rest kwargs)
  ...
  (let* ((table (make-instance 'sqlite-table :db db :name table-name))
