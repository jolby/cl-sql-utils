...
(defun %create-test-table (db &key (table-name "test_table_1"))
  (let* ((create-sql (format nil "CREATE TABLE IF NOT EXISTS ~a (id INTEGER PRIMARY KEY, name TEXT)" table-name)))
...
(defun %create-test-view (db &key (view-name "test_view_1"))
  (let* ((create-sql (format nil "CREATE VIEW IF NOT EXISTS ~a AS SELECT * FROM test_table_1" view-name)))
    (sql-utils:execute db create-sql)))
...
(test creating-db-named-memory-connection
  (let ((db-connection (sql-utils:make-db-connection :sqlite :memory-name "test-memory-db")))
    (is (sql-utils:database-p db-connection))
    (is (sql-utils:open-p db-connection))
    (dbi:disconnect (sql-utils::connection db-connection))
    (is (not (sql-utils:open-p db-connection)))))
...
(test creating-db-file-connection
  ...
  (unwind-protect
       (let ((db-connection (sql-utils:make-db-connection :sqlite :filename "/tmp/test.db")))
         (is (sql-utils:database-p db-connection))
         (is (sql-utils:open-p db-connection))
         (dbi:disconnect (sql-utils::connection db-connection))
         (is (not (sql-utils:open-p db-connection))))
  ...
  (with-test-connection (conn)
    (let ((before-tables (sql-utils:table-names conn)))
      ...
      (let ((after-tables (sql-utils:table-names conn)))
        ...
        (format t "Before tables: ~a, After tables: ~a~%" before-tables after-tables)
      ...
      (let ((before-views (sql-utils:view-names conn)))
        ...
        (with-test-view ()
          (let ((after-views (sql-utils:view-names conn)))
            (format t "Before views: ~a, After views: ~a~%" before-views after-views)
            (assert (= (length after-views) 1))
            (assert (not (null (set-difference after-views before-views))))))))))
...
(test create-table-sql-basic
  (with-test-connection (conn)
    (let ((sql (sql-utils:create-table-sql
                conn "test_table"
...
(test quote-default-value
  (with-test-connection (conn)
    ...
    (is (string= "'foo'" (sql-utils:quote-default-value conn "'foo'")))
    (is (string= "'foo)'" (sql-utils:quote-default-value conn "'foo)'")))
    (is (string= "'1'" (sql-utils:quote-default-value conn "'1'")))
    (is (string= "'1'" (sql-utils:quote-default-value conn "'1'")))
    (is (string= "'1'" (sql-utils:quote-default-value conn "'1'")))
    
    ;; Test expressions
    (is (string= "(STRFTIME('%Y-%m-%d %H:%M:%f', 'NOW'))"
                 (sql-utils:quote-default-value conn "STRFTIME('%Y-%m-%d %H:%M:%f', 'NOW')")))

    ;; Test special SQLite keywords
    (is (string= "CURRENT_TIME" (sql-utils:quote-default-value conn "CURRENT_TIME")))
    (is (string= "CURRENT_DATE" (sql-utils:quote-default-value conn "CURRENT_DATE")))
    (is (string= "CURRENT_TIMESTAMP" (sql-utils:quote-default-value conn "CURRENT_TIMESTAMP")))
    (is (string= "current_timestamp" (sql-utils:quote-default-value conn "current_timestamp")))
    (is (string= "((CURRENT_TIMESTAMP))" (sql-utils:quote-default-value conn "(CURRENT_TIMESTAMP)")))
    ...
    (flet ((create-and-check (col-def initial expected)
             (let ((table-name (format nil "test_table_~A" (random 1000000))))
               ...
               (sql-utils:execute conn
                                  (format nil "CREATE TABLE ~A (col ~A)" table-name col-def))
               ...
               (let* ((table (sql-utils:make-table conn table-name))
                 ...
                 (is (string= expected
                              (sql-utils:quote-default-value conn default-val)))))))
      ...
      (create-and-check "TEXT DEFAULT (STRFTIME('%Y-%m-%d %H:%M:%f', 'NOW'))"
                        ...
                        "(STRFTIME('%Y-%m-%d %H:%M:%f', 'NOW'))")
      (create-and-check "TEXT DEFAULT CURRENT_TIME" "CURRENT_TIME" "CURRENT_TIME")
      (create-and-check "TEXT DEFAULT CURRENT_DATE" "CURRENT_DATE" "CURRENT_DATE")
      (create-and-check "TEXT DEFAULT CURRENT_TIMESTAMP" "CURRENT_TIMESTAMP" "CURRENT_TIMESTAMP")
      (create-and-check "TEXT DEFAULT current_timestamp" "current_timestamp" "current_timestamp")
      (create-and-check "TEXT DEFAULT (CURRENT_TIMESTAMP)" "CURRENT_TIMESTAMP" "CURRENT_TIMESTAMP")
      (create-and-check "TEXT DEFAULT 'CURRENT_TIMESTAMP'" "'CURRENT_TIMESTAMP'" "'CURRENT_TIMESTAMP'")
      (create-and-check "TEXT DEFAULT \"CURRENT_TIMESTAMP\"" "\"CURRENT_TIMESTAMP\"" "\"CURRENT_TIMESTAMP\""))))
...
(test row-count-basics
  (with-test-connection (conn)
    (let ((table (sql-utils:create-table
      ...
      (sql-utils:execute conn "INSERT INTO test_table (name) VALUES ('Charlie')")
      
      ;; Count should now be 3
      (is (= 3 (sql-utils:row-count table)))
      
      ;; Test count-where
      (is (= 1 (sql-utils:row-count-where table :where "name = ?" :where-args '("Alice"))))
      (is (= 2 (sql-utils:row-count-where table :where "name LIKE ?" :where-args '("%i%"))))
      (is (= 0 (sql-utils:row-count-where table :where "name = ?" :where-args '("David")))))))
...
(test rows-basics
  (with-test-connection (conn)
    (let ((table (sql-utils:create-table
      ...
      (is (null (sql-utils:rows table)))
      
      ;; Insert test data
      (sql-utils:execute conn "INSERT INTO test_table (name, age) VALUES ('Alice', 25)")
      (sql-utils:execute conn "INSERT INTO test_table (name, age) VALUES ('Bob', 30)")
      (sql-utils:execute conn "INSERT INTO test_table (name, age) VALUES ('Charlie', 35)")
      (sql-utils:execute conn "INSERT INTO test_table (name, age) VALUES ('David', 40)")
      (sql-utils:execute conn "INSERT INTO test_table (name, age) VALUES ('Eve', 45)")
      
      ;; Test basic row retrieval
      (let ((all-rows (sql-utils:rows table)))
        (is (= 5 (length all-rows)))
        (is (string= "Alice" (getf (first all-rows) :|name|))))
      
      ;; Test rows-where with WHERE clause
      (let ((young-people (sql-utils:rows-where table 
                                               :where "age < ?" 
                                               :where-args '(35))))
      ...
      (let ((limited-rows (sql-utils:rows-where table 
                                               :order-by "age"
                                               :limit 2
                                               :offset 1)))
  ...
  (with-test-connection (conn)
    (let ((table (sql-utils:create-table
      ...
      (let ((record '(:name "Alice" :age 30)))
        ...
        (let ((row (first (sql-utils:rows table))))
  ...
  (with-test-connection (conn)
    (let ((table (sql-utils:create-table
      ...
      (let ((records (list
        ...
        (is (= 3 (sql-utils:row-count table)))
        (let ((rows (sql-utils:rows table :order-by "age")))
...
(test lookup-basics
  (with-test-connection (conn)
    (let ((table (sql-utils:create-table
      ...
      (signals sql-utils:not-found-error (sql-utils:lookup table 999))
      ;; Test value-or-default for non-existing record
      (is (equal :not-found (sql-utils:value-or-default table 999 :not-found)))
      )))
...
(test transform-basics
  (with-test-connection (conn)
    (let ((table (sql-utils:create-table
      ...
      (is (not (set-difference (sql-utils:column-names table) (list "id" "name" "age") :test #'equal)))
...
(test add-column-basics
  (with-test-connection (conn)
    (let ((table (sql-utils:create-table
      ...
      (let ((status-col (find "status" (sql-utils:columns table)
                             :key #'sql-utils:name :test #'string=)))
      ...
      (sql-utils:add-column table "parent_id"
                           ...
                           :fk "test_add_column"
                           :fk-col "id")
      (let ((fk (find "parent_id" (sql-utils:foreign-keys table)
                      :key #'sql-utils:column :test #'string=)))
...
(test add-foreign-key-basics
  (with-test-connection (conn)
    ;; Create the tables
    (let ((parent (sql-utils:create-table
                   conn "parent"
                   '(("id" . "INTEGER")
                     ("name" . "TEXT"))
                   :pk "id"))
          (child (sql-utils:create-table
                  conn "child" 
                  '(("id" . "INTEGER")
                    ("parent_id" . "INTEGER")
                    ("name" . "TEXT"))
      ...
      (signals sql-utils:missing-db-type-error
        (sql-utils:add-foreign-key child "parent_id" :other-table "table_nonexistent"))
...
(test drop-table-basics
  (with-test-connection (conn)
    ...
    (let ((table (sql-utils:create-table
      ...
      ;; Verify table exists
      (is (sql-utils:exists-p table))
      (is (member "test_drop" (sql-utils:table-names conn) :test #'string=))
      ...
      (is (not (member "test_drop" (sql-utils:table-names conn) :test #'string=)))
      ...
      (finishes (sql-utils:drop table :ignore t)))))
      ...
      (let ((fks (sql-utils:foreign-keys child)))
        ...
        (let ((fk (find "parent_id" fks :key #'sql-utils:column :test #'string=)))
          (is (string= "parent" (sql-utils:other-table fk)))
          (is (string= "id" (sql-utils:other-column fk))))
        ;; Verify second foreign key  
        (let ((fk (find "other_id" fks :key #'sql-utils:column :test #'string=)))
...
(test transform-with-foreign-keys
  (with-test-connection (conn)
    ...
    (let ((table (sql-utils:create-table
      ...
      (is (not (set-difference (list "parent_id") (sql-utils:fk-names table) :test #'equal)))
      ...
      (is (not (set-difference (list "parent_id" "other_id") (sql-utils:fk-names table) :test #'equal)))
